...
export default class ReactGenerator {
  ...
  async generateComponent(spec, options = {}) {
    try {
      ...
      const validation = await validateCode(componentCode, 'react');
      
      // Formatage du code
      const formattedCode = await formatCode(componentCode, 'react');
  ...
  async generateHook(spec, options = {}) {
    try {
      ...
      const validation = await validateCode(hookCode, 'react');
      const formattedCode = await formatCode(hookCode, 'react');
  ...
  async generateContext(spec, options = {}) {
    try {
      ...
      // Validation et formatage
      const validation = await validateCode(contextCode, 'react');
      const formattedContext = await formatCode(contextCode, 'react');
      const formattedProvider = await formatCode(providerCode, 'react');
      const formattedHook = await formatCode(hookCode, 'react');
