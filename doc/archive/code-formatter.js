...
class CodeFormatter {
    ...
    async formatCode(code, language = 'javascript', context = {}) {
        ...
        const prettier = availableTools.find(t => t.name === 'prettier' && t.available);
        if (prettier) {
        ...
        // Fallback sur ESLint
        const eslint = availableTools.find(t => t.name === 'eslint' && t.available);
        if (eslint) {
    ...
    async formatAndValidate(code, context = {}) {
        ...
        const validationResult = await validator.validateCode(formatResult.code, context);
        ...
        for (let i = 0; i < maxLines; i++) {
            ...
            if (originalLine.trim() !== formattedLine.trim()) {
                changes.push({
                    ...
                    type: originalLine === '' ? 'added' : formattedLine === '' ? 'removed' : 'modified'
                });
            }
        }
