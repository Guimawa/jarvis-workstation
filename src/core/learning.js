...
export default class LearningSystem extends EventEmitter {
  ...
  async selectBestStrategy(strategies, analysis) {
    try {
      ...
      const sortedStrategies = Array.from(scores.values()).sort(
        (a, b) => b.score - a.score,
      );
      ...
      if (bestStrategy) {
        this.logger.debug(
          `🎯 Meilleure stratégie sélectionnée: ${bestStrategy.name}`,
        );
      }
  ...
  async calculateStrategyScore(strategy, analysis) {
    ...
    const contextScore = await this.calculateContextualScore(
      strategy,
      analysis,
    );
  ...
  updatePattern(category, pattern, experience) {
    ...
    if (experience.success) {
      patternData.successRate =
        (patternData.successRate * (patternData.occurrences - 1) + 1) /
        patternData.occurrences;
      patternData.averageQuality =
        (patternData.averageQuality * (patternData.occurrences - 1) +
          experience.quality) /
        patternData.occurrences;
      patternData.averageTime =
        (patternData.averageTime * (patternData.occurrences - 1) +
          experience.responseTime) /
        patternData.occurrences;
    } else {
      patternData.successRate =
        (patternData.successRate * (patternData.occurrences - 1)) /
        patternData.occurrences;
    }
  ...
  async updateStrategyEffectiveness(experience) {
    ...
    if (experience.success) {
      strategyData.successRate =
        (1 - alpha) * strategyData.successRate + alpha * 1;
      strategyData.averageQuality =
        (1 - alpha) * strategyData.averageQuality + alpha * experience.quality;
    } else {
      strategyData.successRate =
        (1 - alpha) * strategyData.successRate + alpha * 0;
    }

    strategyData.averageTime =
      (1 - alpha) * strategyData.averageTime + alpha * experience.responseTime;
  }
  ...
  async identifyOptimizations(experience) {
    ...
    if (experience.responseTime > 5000 && experience.quality > 0.8) {
      ...
      this.recordOptimization(optimizationKey, {
        ...
        strategy: experience.strategy,
        suggestion:
          "Considérer une approche plus directe pour ce type de requête",
        impact: "high",
      });
    }
  ...
  async identifyPatternOptimizations(experience) {
    ...
    if (similarExperiences.length >= 5) {
      const averageQuality =
        similarExperiences.reduce((sum, exp) => sum + exp.quality, 0) /
        similarExperiences.length;
      const averageTime =
        similarExperiences.reduce((sum, exp) => sum + exp.responseTime, 0) /
        similarExperiences.length;
  ...
  async adaptBehavior(experience) {
    ...
    const applicableOptimizations = Array.from(
      this.knowledge.optimizations.values(),
    ).filter(
      (opt) => opt.confidence > this.config.adaptationThreshold && !opt.applied,
    );
  ...
  calculateSimilarity(exp1, exp2) {
    ...
    const complexityDiff = Math.abs(
      (exp1.analysis?.complexity || 0) - (exp2.analysis?.complexity || 0),
    );
  ...
  addExperience(experience) {
    ...
    if (this.history.experiences.length > this.config.memoryWindow) {
      this.history.experiences = this.history.experiences.slice(
        -this.config.memoryWindow,
      );
    }
  }
  ...
  async loadKnowledge() {
    try {
      ...
      this.logger.debug(
        "Aucune connaissance existante trouvée, démarrage à zéro",
      );
    }
  }
  ...
  async analyzeExistingPatterns() {
    const significantPatterns = Array.from(this.knowledge.patterns.values())
      .filter(
        (pattern) => pattern.occurrences >= this.config.patternMinOccurrence,
      )
      .sort((a, b) => b.successRate - a.successRate);

    this.logger.info(
      `📊 ${significantPatterns.length} patterns significatifs identifiés`,
    );
  ...
  /**
   * Optimise un prompt basé sur l'apprentissage
   */
  async optimizePrompt(prompt) {
    try {
      // Recherche de patterns similaires dans l'historique
      const similarExperiences = this.history.experiences
        .filter((exp) => exp.success && exp.quality > 0.8)
        .slice(-10);

      if (similarExperiences.length > 0) {
        // Utiliser les patterns qui ont bien fonctionné
        const successfulPatterns = similarExperiences
          .map((exp) => exp.request)
          .join("\n");

        return `${prompt}\n\nPatterns similaires réussis:\n${successfulPatterns}`;
      }

      return prompt;
    } catch (error) {
      console.warn("Erreur optimisation prompt:", error.message);
      return prompt;
    }
  }

  /**
   * Enregistre le résultat d'une génération
   */
  async recordGenerationResult(prompt, result) {
    try {
      const experience = {
        id: this.generateExperienceId(),
        timestamp: Date.now(),
        request: { type: "generation", prompt },
        result,
      ...
      console.warn("Erreur enregistrement résultat:", error.message);
    }
  }
  ...
  async recordError(errorData) {
    try {
      ...
      console.warn("Erreur enregistrement erreur:", error.message);
    }
  }
...
const learningLogs = [
  {
    source: "ReactGenerator",
    message: "Amélioration du prompt de génération : ajout des hooks auto.",
    date: "2025-09-04T13:00:00Z",
  },
  {
    source: "MemorySystem",
    message: "Suppression des duplications de composants similaires.",
    date: "2025-09-03T15:22:00Z",
  },
  {
    source: "BrainSystem",
    message:
      "Optimisation de la stratégie de génération pour les composants TypeScript.",
    date: "2025-09-03T10:15:00Z",
  },
  {
    source: "CodeValidator",
    message: "Apprentissage de nouveaux patterns d'erreurs React.",
    date: "2025-09-02T16:45:00Z",
  },
  {
    source: "LearningSystem",
    message:
      "Découverte d'un pattern récurrent : composants avec hooks personnalisés.",
...
export function addLearningLog(source, message) {
